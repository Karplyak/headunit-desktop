/**
 * @mainpage RDA5807M Arduino Library implementation
 * @details RDA5807M Arduino Library implementation. This is an Arduino library for the RDA5807M, BROADCAST RECEIVER.
 * @details It works with I2C protocol and can provide an easier interface to control the RDA5807M device.<br>
 * @details This library was built based on "RDA5807MM - SINGLE-CHIP BROADCAST FM RADIO TUNER - Rev.1.1Aug.2015"
 * @details and RDA microelectronics RDA5807MFP - SINGLE-CHIP BROADCAST FM RADIO TUNER
 ...
 */
void RDA5807M::setGpio(uint8_t gpioPin, uint8_t gpioSetup, int mcuPin)
{
    switch (gpioPin)
    {
    case 1:
 ...
 * @see setGpio
 */
void RDA5807M::setInterruptMode(uint8_t value)
{
 ...
 * @see RDA5807MM - SINGLE-CHIP BROADCAST FMRADIO TUNER; pages 5, 9, 12 and 13.
...
void RDA5807M::getStatusRegisters()
{
    word16_to_bytes aux;
    int i, j;
    //qWarning("RDA5804m getStatusRegisters !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
/*  ////
    ...
    int fd;
        ...
        if (ioctl(fd, I2C_SLAVE, this->deviceAddressFullAccess) < 0) {
            qWarning("Could not set address to 0x%02x : %s", this->deviceAddressDirectAccess, strerror(errno));
            return;
        }

        //read light value
        //if (m_sensorSettings["alsRegisterWidth"].toInt() == 16)        else result = i2c_smbus_read_byte_data(file, alsReg);
        int result = 0;
        char buf[12] = {0};
        ...
        //if (result < 0)
        {
            qWarning("Unable to write register data byte 0x%02x: %s", this->deviceAddressDirectAccess, strerror(errno));
            return;
        }
        //qWarning("RDA5807M::getStatusRegisters buf: 0x%02x 0x%02x: 0x%02x 0x%02x: 0x%02x 0x%02x", buf[0], buf[1],buf[2], buf[3],buf[4], buf[5]);
        j=0;
        for (i = 0; i < 6; i++)
            {
                //result = i2c_smbus_read_byte(fd);

                aux.refined.highByte = buf[0+j];
                aux.refined.lowByte = buf[1+j];
                j++;j++;
                shadowStatusRegisters[i] = aux.raw;
                //qWarning("RDA5807M::getStatusRegisters[%d]: 0x%04x",i, aux.raw);
            }
    ...
    //usleep(20000);
        //qWarning("RDA5807M::getStatusRegisters: 0x%04x", result);

       /* aux.refined.highByte = Wire.read();
        aux.refined.lowByte = Wire.read();
    aux.raw = result;
    result = aux.refined.lowByte <<8;
    result += aux.refined.highByte;*/
    //qWarning("RDA5807M::getStatus: 0x%04x", result);
    aux.raw = result;
 ...
 * @see  word16_to_bytes datatype in RDA5807M.h
 */
word16_to_bytes RDA5807M::getDirectRegister(uint8_t reg)
{

    word16_to_bytes aux;
    qWarning("RDA5804m getDirectRegister !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
    /* ////
 ...
 */
void *RDA5807M::getStatus(uint8_t reg)
{
    word16_to_bytes aux;
...
/* ////
    ...
    int fd;
        ...
        if (ioctl(fd, I2C_SLAVE, this->deviceAddressDirectAccess) < 0) {
            qWarning("Could not set address to 0x%02x : %s", this->deviceAddressDirectAccess, strerror(errno));
            return 0;
        }
        ...
        if (result < 0)
        {
            qWarning("Unable to write register data byte 0x%02x::0x%02x : %s", this->deviceAddressDirectAccess, reg, strerror(errno));
            return 0;
        }

        close(fd);

    //usleep(20000);         qWarning("RDA5807M::getStatus: 0x%04x", result);
 ...
 *
 * @see RDA5807MM - SINGLE-CHIP BROADCAST FMRADIO TUNER; pages 5, 9, 10 and 11.
...
int result = 0;
    ...
    int fd;
        ...
        result = aux.refined.lowByte <<8;
        result += aux.refined.highByte;
        //qWarning("RDA5807M::getStatus: 0x%04x :0x%02x 0x%02x", value, aux.refined.highByte, aux.refined.lowByte);


        if (i2c_smbus_write_word_data(fd, reg, result)< 0)
        {
            qWarning("Unable to write register data block to 0x%02x::0x%02x : %s", this->deviceAddressDirectAccess, reg, strerror(errno));
            return;
        }
/*
        if (i2c_smbus_write_byte(fd,   aux.refined.lowByte) < 0)
              {
                  qWarning("Unable to write register data byte 0x%02x: %s", this->deviceAddressDirectAccess,  strerror(errno));
                  return;
              }
 ...
 */
void RDA5807M::setNewDemodulateMethod(bool value)
{
 ...
 */
void RDA5807M::setup(uint8_t clock_frequency, uint8_t oscillator_type, uint8_t rlck_no_calibrate)
{
    ...
    usleep(100); //delay(10);
    powerUp();
    usleep(this->maxDelayAftarCrystalOn); //delay(this->maxDelayAftarCrystalOn);
 ...
 * @param value  true or false
 */
void RDA5807M::setSoftBlendEnable(bool value)
{
 ...
 * @param value  true or false
 */
void RDA5807M::setAFC(bool value)
{
 ...
 * @see RDA5807MM - SINGLE-CHIP BROADCAST FM RADIO TUNER - Rev.1.1Aug.2015; pages 9 and 12.
 *
 * @param channel
 */
 ...
 * @param frequency
 */
void RDA5807M::setDirectFrequency(uint16_t frequency)
{
 ...
 */
void RDA5807M::setFrequencyMode(uint8_t value)
{
 ...
 */
void RDA5807M::setBand3_50_65_Mode(uint8_t band3Mode)
{
 ...
 */
uint8_t RDA5807M::getBand3Status()
{
    rda_reg07 tmp;
 ...
 */
void RDA5807M::setStep(uint8_t step)
{
    uint8_t space;
    switch (step)
    {
 ...
 * @details Default is 75 �s (used in USA); 50 �s is used in Europe, Australia, Japan.
 * @code {.cpp}
 * #include <RDA5807M.h>
 * RDA5807M rx;
 * void setup() {
 *  rx.setup();
 *  rx.setFrequency(10390); // Tunes at 103.9 MHz
 *  rx.setFmDeemphasis(1); // Sets to 50 �s. Used in Europe, Australia, Japan.
 * }
 * void loop() {
 * }
 * @endcode
 *
 * @param de  0 = 75 �s; 1 = 50 �s
 */
 ...
 */
void RDA5807M::setRBDS(bool value)
{
    this->oldTextABFlag = reg02->refined.SEEK = 0;
 ...
 * @see getRdsReady
 */
uint8_t RDA5807M::getRdsFlagAB(void)
{
    rds_blockb blkb;
 ...
 * @return True or false
 */
bool RDA5807M::isNewRdsFlagAB(void)
{
    rds_blockb blkb;
    blkb.blockB = reg0d->RDSB;
 ...
 */
bool RDA5807M::getRdsAllData(char **stationName, char **stationInformation, char **programInformation, char **utcTime) {
 ...
 * @see getRdsReady
 */
uint16_t RDA5807M::getRdsGroupType()
{
    rds_blockb blkb;
    blkb.blockB = reg0d->RDSB;
 ...
 * @see getRdsReady
 */
uint8_t RDA5807M::getRdsVersionCode(void)
{
    rds_blockb blkb;
    blkb.blockB = reg0d->RDSB;
 ...
 */
uint8_t RDA5807M::getRdsProgramType(void)
{
    rds_blockb blkb;
    blkb.blockB = reg0d->RDSB;
 ...
 */
uint8_t RDA5807M::getRdsTrafficProgramCode(void)
{
    rds_blockb blkb;
    blkb.blockB = reg0d->RDSB;
 ...
 */
void RDA5807M::getNext2Block(char *c)
{
    char raw[2];
    int i, j;
    word16_to_bytes blk;
 ...
 */
void RDA5807M::getNext4Block(char *c)
{
    char raw[4];
    int i, j;
 ...
 * @see getRdsReady
 */
char *RDA5807M::getRdsText0A(void)
{
    ...
    if (blkb.group0.groupType == 0)
    {
        ...
        if (rdsTextAdress0A >= 0 && rdsTextAdress0A < 4)
        {
            ...
            rds_buffer0A[8] = '\0';
           // qWarning("RDA5807M::getRdsText0A: %d :%s ", rdsTextAdress0A, rds_buffer0A);
            return rds_buffer0A;
        }
    }
    return NULL;
 ...
 * @see getRdsReady
 */
char *RDA5807M::getRdsText2A(void)
{
 ...
 * @see getRdsReady
 */
char *RDA5807M::getRdsText2B(void)
{
 ...
 * @return true  or false
 */
void RDA5807M::clearRdsFifo(bool value)
{
 ...
 */
void RDA5807M::convertToChar(uint16_t value, char *strValue, uint8_t len, uint8_t dot, uint8_t separator, bool remove_leading_zeros)
{
    char d;
 ...
 */
int RDA5807M::checkI2C(uint8_t *addressArray)
{
    //Wire.begin();
    int error, address;
    int idx = 0;
    ...
    {
        error = ioctl(fd, I2C_SLAVE, address);
        if (error == 0)
        {
 ...
 * @section  GA06 I2S
 * @details  When setting I2S_ENABLE (register 04) bit is high, the RDA5807MFP can get the output signals SCK, WS, SD signals from GPIO3, GPIO1 and GPIO2 (I2S master)
 */
 ...
 *
 * @see RDA microelectronics RDA5807MFP - SINGLE-CHIP BROADCAST FM RADIO TUNER pages 11 and 12
 *
 * @see setI2SOn
 */
void RDA5807M::setI2SAllParameters(uint8_t R_DELY, uint8_t L_DELY, uint8_t SCLK_O_EDGE, uint8_t SW_O_EDGE, uint8_t I2S_SW_CNT, uint8_t WS_I_EDGE, uint8_t DATA_SIGNED, uint8_t SCLK_I_EDGE, uint8_t WS_LR, uint8_t SLAVE_MASTER, uint8_t OPEN_MODE)
{
 ...
 * @details  When setting I2S_ENABLE (register 04) bit is high, the RDA5807MFP you can get the output signals SCK, WS, SD signals from GPIO3, GPIO1 and  GPIO2 (I2S master)
 *
 * @param value  true or false
 */
void RDA5807M::setI2SOn(bool value)
{
 ...
 * @param value  true or false
 */
void RDA5807M::setI2SMaster(bool value)
{
 ...
 * @param value value
 */
void RDA5807M::setI2SSpeed(uint8_t value)
{
 ...
 */
void RDA5807M::setI2SDataSigned(bool value)
{
 ...
 */
void RDA5807M::setAudioOutputHighImpedance(bool value)
{
    reg02->refined.SEEK = 0;
 ...
 */
void RDA5807M::setLnaIcSel(uint8_t value)
{
 ...
 */
void RDA5807M::setLnaPortSel(uint8_t value)
{
